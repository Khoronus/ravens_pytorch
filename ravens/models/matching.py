...
class Matching:
  ...
  def __init__(self,
    ...
    self.optim = torch.optim.Adam(lr=1e-5)
  ...
  def forward(self, input_image):
    ...
    input_tensor = torch.from_numpy(input_data)#tf.convert_to_tensor(input_data, dtype=tf.float32)

    # Rotate input.
    pivot = np.array(input_data.shape[1:3]) / 2
    rvecs = self.get_se2(self.num_rotations, pivot)
    input_tensor = input_tensor.repeat(repeats=self.num_rotations, axis=0)#tf.repeat(input_tensor, repeats=self.num_rotations, axis=0)
    #input_tensor = tfa_image.transform(input_tensor, rvecs, interpolation='NEAREST')
    #input_tensor = torch.nn.functional.interpolate(    # <------------ WRONG
    #    input_tensor, rvecs, mode='nearest')

    # Forward pass.
    input_tensor = torch.split(input_tensor, self.num_rotations)
    logits = ()
    ...
    #logits = torch.nn.functional.interpolate(logits, rvecs, mode='nearest') # <--------------- WRONG
    c0 = self.padding[:2, 0]
  ...
  def train(self, input_image, p, q, theta):
    ...
    if True:
      ...
      positive_distances = torch.reshape(positive_distances, (1,))
      positive_labels = torch.ones([1], dtype=torch.int32)#, device=cuda0)#tf.tensor(tf.constant([1], dtype=tf.int32)
      positive_loss = losses.ContrastiveLoss(positive_labels,
                                                  positive_distances)
      ...
      for i in range(num_samples):
        ...
        negative_labels += torch.ones([0], dtype=torch.int32)#, device=cuda0)#tf.tensor(tf.constant([1], dtype=tf.int32)(tf.constant([0], dtype=tf.int32),)
      negative_distances = torch.cat(negative_distances, axis=0)
      negative_labels = torch.cat(negative_labels, axis=0)
      negative_loss = losses.ContrastiveLoss(negative_labels,
                                                  negative_distances)
      negative_loss = torch.mean(negative_loss)

      loss = torch.mean(positive_loss) + torch.mean(negative_loss)
