...
def pick_max_mean(pi, mu, var):
  ...
  """
  del var
  mu = torch.reshape(mu, (mu.shape()[0], pi.shape()[1], -1))
  d_out = mu.shape()[-1]
  batch_size, _ = pi.shape
  prediction = np.zeros((batch_size, d_out))
  argmax_pi = torch.argmax(pi, axis=1)  # shape (batch_size)
...
def multivar_gaussian_pdf(y, mu, var):
  ...
  y = y.unsqueeze(1)
  d = mu.shape[-1]
  dot_prod = torch.sum((y - mu)**2, (2))  # shape (batch_size, k)
  exp_factor = torch.div(-1., (2. * (var))) * dot_prod
  numerator = torch.exp(exp_factor)  # shape (batch_size, k)
  denominator = torch.sqrt((2 * np.pi * (var))**d)
  return torch.multiply_no_nan(numerator, 1 / denominator)
...
def mdn_loss(y, mdn_predictions):
  ...
  p = torch.multiply(
      torch.clamp(pdf, 1e-8, 1e8), torch.clamp(pi, 1e-8, 1e8))
  p = torch.sum(p, axis=1, keepdims=True)
  p = -torch.log(torch.clamp(p, 1e-8, 1e8))
