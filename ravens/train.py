...
#flags.DEFINE_integer('n_steps', 50, '')
...
def main(unused_argv):
  ...
  for train_run in range(FLAGS.n_runs):
    ...
    log_dir = os.path.join(FLAGS.train_dir, 'logs', FLAGS.agent, FLAGS.task,
                           curr_time, 'train')
    writer = SummaryWriter(log_dir)#tf.summary.create_file_writer(log_dir)

    # Initialize agent.
    np.random.seed(train_run)
    torch.manual_seed(train_run) # tf.random.set_seed(train_run)
