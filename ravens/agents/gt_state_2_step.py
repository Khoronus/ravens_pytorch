...
class GtState2StepAgent(GtStateAgent):
  ...
  def __init__(self, name, task):
    ...
    self.pick_optim = torch.optim.Adam(lr=2e-4)
    self.place_optim = torch.optim.Adam(lr=2e-4)
    self.metric = torch.mean#tf.keras.metrics.Mean(name='metric')
    self.val_metric = torch.mean#tf.keras.metrics.Mean(name='val_metric')
  ...
  def train(self, dataset, num_iter, writer, validation_dataset):
    ...
    if self.use_mdn:
      ...
      loss_criterion = torch.nn.MSELoss#tf.keras.losses.MeanSquaredError()
    ...
    def train_step(pick_model, place_model, batch_obs, batch_act,
      ...
      if True:
        ...
        #                                         stddev=0.001)), axis=1)
        batch_obs = torch.cat((batch_obs, batch_act[:, 0:3]), axis=1)
    ...
    for i in range(num_iter):
      ...
      if i % print_rate == 0:
        ...
        print(f'glstate2A::Train Iter: {self.total_iter + i} Loss: {loss:.4f} Iter time:',
              time.time() - start)
...
class GtState3Step6DAgent(GtState6DAgent):
  ...
  def __init__(self, name, task):
    ...
    self.pick_optim = torch.optim.Adam(lr=2e-4)
    self.place_se2_optim = torch.optim.Adam(lr=2e-4)
    self.place_rpz_optim = torch.optim.Adam(lr=2e-4)

    self.metric = torch.mean
    self.val_metric = torch.mean
  ...
  def train(self, dataset, num_iter, writer, validation_dataset):
    ...
    for i in range(num_iter):
      ...
      if i % print_rate == 0:
        loss = np.float32(loss)
        print(f'glstate2B::Train Iter: {self.total_iter + i} Loss: {loss:.4f} Iter time:',
              time.time() - start)
        # utils.meshcat_visualize(self.vis, obs, act, info)
