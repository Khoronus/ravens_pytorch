...
class Transporter6dAgent(TransporterAgent):
  ...
  def __init__(self, name, task):
    ...
    self.p0_pixel_error = torch.mean
    self.p1_pixel_error = torch.mean
    self.p0_theta_error = torch.mean
    self.p1_theta_error = torch.mean
    self.metrics = [
  ...
  def train(self, dataset, num_iter, writer, validation_dataset):
    ...
    for i in range(num_iter):
      ...
      print(f'Transporter6dof::Train Iter: {self.total_iter + i} Loss: {loss0:.4f} {loss1:.4f}')

      if (self.total_iter + i) % validation_rate == 0:
        print('Validating!')
        #tf.keras.backend.set_learning_phase(0)
        self.transport_model.eval()
        self.rpz_model.eval()
        ...
        self.transport_model.train()
        self.rpz_model.train()
