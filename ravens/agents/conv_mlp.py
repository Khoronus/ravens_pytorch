...
class ConvMlpAgent:
  ...
  def train(self, dataset, num_iter, writer, validation_dataset):
    ...
    #@tf.function # no required in pytorch (graph by default)
    ...
    for i in range(num_iter):
      ...
      print(f'convmpl::Train Iter: {self.total_iter + i} Loss: {loss0:.4f} Iter time:',
            time.time() - start)
...
def main():
  agent = ConvMlpAgent('demo_debug', 'sweeping')
  ppa = PickPlaceConvMlpAgent(agent)
